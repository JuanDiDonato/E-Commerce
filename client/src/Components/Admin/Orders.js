...
export default function Orders(props) {
     ...
     const change_status = (id_order, id) => {
          let status
          if(id === 1) status = 2
          else if(id === 2) status = 3
          AdminServices.change_status(id_order, status)
          AdminServices.get_orders().then(data => {
               setOrders(data)
          })
     }
                                        ...
                                        orders.map(order => {
                                             return (
                                                  <Tr key={order.id_order}>
                                                       ...
                                                       <Td scope="col">{order.waist}</Td>
                                                       <Td scope="col">{order.Status.status}</Td>
                                                       {order.Status.id === 1 ? <Td scope="col" className="btn-order" style={{cursor:'pointer'}} onClick={() => change_status(order.id_order, order.Status.id)}>Marcar como enviado</Td>
                                                       : 
                                                       <div>
                                                            {order.Status.id === 2 ? <Td scope="col" className="btn-order" style={{cursor:'pointer'}} onClick={() => change_status(order.id_order, order.Status.id)}>Marcar como entregado</Td>
