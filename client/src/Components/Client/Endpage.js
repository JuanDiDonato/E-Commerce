...
export default function Endpage(props) {
     ...
     const push_order = () => {
          ...
          cart.forEach(product => {
               ...
               const ProductData = { 'id_product': product.id_product, quantity: product.quantity, 'photo': product.Product.photo, 'title': product.Product.title, 'address': user.address, 'waist' : product.waist }
               ProductService.add_order(ProductData)
               ProductService.save_in_history(ProductData)
               product.Product.Waist[product.waist] = product.Product.Waist[product.waist] - product.quantity
               ProductService.stock(product.id_product, product.Product.Waist)
          });
     ...
     return (
          <div>
               ...
               <h4>El vendedor enviara estos productos a {user.address}</h4>
               <div>
